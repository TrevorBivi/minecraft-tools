    ...
    if name in kn:
        ...
        try:
            ...
            try:
                ...
                im = Image.open(texture_dir + 'dragon_egg.png')
        kn[name] = im
    return im
...
def get_biv_tile_pos(block,tl):
    return (block.absolute_coords[0] - tl[0] * 32 * 16) * 16,(block.absolute_coords[1] - tl[1] * 32 * 16) * 16
                ...
                for z in range(16):
                    ...
                    for y in range(255,0,-1):
                        if (x,y,z) in ch.blocks and ch.blocks[(x,y,z)].name and ch.blocks[(x,y,z)].name != b'minecraft:air':
                            ...
                            for npx in range(16 ):
                                for npy in range(16):
                                    p_coords =  get_biv_tile_pos(ch.blocks[(x,y,z)], reg_edges)  # * 32  + c[1] * 16 + z
